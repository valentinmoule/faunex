...
const targets: { id: ShareTarget; label: string; icon: typeof Instagram; className: string }[] = [
  { id: 'stories', label: 'Stories', icon: Instagram, className: 'bg-gradient-to-br from-[hsl(330,85%,55%)] to-[hsl(35,95%,55%)] text-white' },
  { id: 'instagram', label: 'Instagram', icon: Instagram, className: 'bg-gradient-to-br from-[hsl(280,70%,55%)] to-[hsl(340,85%,55%)] text-white' },
  { id: 'facebook', label: 'Facebook', icon: Share2, className: 'bg-[hsl(221,60%,45%)] text-white' },
  { id: 'whatsapp', label: 'WhatsApp', icon: MessageCircle, className: 'bg-[hsl(142,60%,40%)] text-white' },
];
...
const ShareCaptureSheet = ({ card, open, onClose }: Props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error(err);
        if (!cancelled) toast({ title: 'Aperçu indisponible', description: "L'image n'a pas pu être générée.", variant: 'destructive' });
      }
    })();
    return () => {
      cancelled = true;
          ...
          toast({
            ...
            description:
              target === 'stories' || target === 'instagram'
                ? 'Ouvre Instagram et importe l’image depuis ta galerie.'
                : 'Ta carte est prête à être partagée.',
          });
        }
        if (target === 'download' || result === 'shared') onClose();
      } catch (err) {
        console.error(err);
        toast({ title: 'Partage impossible', description: 'Réessaie dans un instant.', variant: 'destructive' });
      } finally {
        setPending(null);
      }
    },
  ...
  const handleCopyLink = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: 'Lien copié', description: 'Colle-le où tu veux.' });
    } catch {
        ...
        <Drawer.Content className="fixed inset-x-0 bottom-0 z-[71] max-h-[92vh] rounded-t-3xl bg-background pb-[calc(1.5rem+env(safe-area-inset-bottom))] outline-none">
          ...
          <div className="px-5 pt-4 space-y-5 overflow-y-auto">
            <div className="text-center">
              <h2 className="font-display text-lg font-bold">Partager ta carte</h2>
            ...
            <div className="mx-auto w-40 aspect-[4/5] rounded-2xl overflow-hidden bg-muted flex items-center justify-center shadow-lg">
              {previewUrl ? (
                <img src={previewUrl} alt={`Aperçu de la carte ${card.name}`} className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="grid grid-cols-4 gap-3">
              {targets.map(({ id, label, icon: Icon, className }) => (
                <button
                  key={id}
