...
BEGIN
  ...
  IF r_name IS NULL AND n_name IS NOT NULL AND length(n_name) >= 5 THEN
    ...
    WHERE t.merged_into IS NULL
      ...
      AND extensions.similarity(public.normalize_animal_label(t.vernacular_name), n_name) >= 0.86
  ...
  IF r_name IS NULL AND n_name IS NOT NULL AND length(n_name) >= 5 THEN
    ...
    WHERE left(public.normalize_animal_label(a.name), 3) = left(n_name, 3)
      AND extensions.similarity(public.normalize_animal_label(a.name), n_name) >= 0.86
