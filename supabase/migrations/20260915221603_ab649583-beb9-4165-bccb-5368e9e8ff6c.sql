...
as $$
  with bounds as (
    ...
    where c.status = 'approved'
      and (
        coalesce(p_period, 'week') = 'all'
        or (c.created_at >= b.week_start and c.created_at < b.week_start + interval '7 days')
      )
      and (
...
revoke all on function public.category_leaderboard(text, integer, text, text) from public, anon;
revoke all on function public.my_category_rank(text, text, text) from public, anon;
grant execute on function public.category_leaderboard(text, integer, text, text) to authenticated;
grant execute on function public.my_category_rank(text, text, text) to authenticated;