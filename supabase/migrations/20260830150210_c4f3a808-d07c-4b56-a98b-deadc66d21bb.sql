...
CREATE UNIQUE INDEX IF NOT EXISTS ai_analysis_attempts_user_request_unique
ON public.ai_analysis_attempts (user_id, request_id)
...
BEGIN
  ...
  v_limit := CASE WHEN public.is_premium(p_user) THEN 60 ELSE 4 END;
...
CREATE OR REPLACE FUNCTION public.refund_ai_analysis(p_user uuid, p_request uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
REVOKE ALL ON FUNCTION public.refund_ai_analysis(uuid, uuid) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.refund_ai_analysis(uuid, uuid) TO service_role;